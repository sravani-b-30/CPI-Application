...
def get_snowflake_connection():
        conn = snowflake.connector.connect(
             user='SRAVANI30',
             password='Sravani@2003',
             account='CSAHCFY.RV96822',
             warehouse='COMPUTE_WH',
             database='CPI_DATABASE',
             schema='SEPTEMBER_DATA',
             role='ACCOUNTADMIN',
             login_timeout=60
        )
        return conn
...
def load_and_preprocess_data():
    ...
    # Load 'final_scraped_mattress_updated' data
    scraped_query = "SELECT * FROM PRODUCT_DETAILS;"
    df_scrapped = load_data_from_snowflake(scraped_query)
    serp_query = "SELECT * FROM TITLE_DATA;"
    df_serp = load_data_from_snowflake(serp_query)
    df2_query = "SELECT * FROM SEPT_DATA;"
    df2 = load_data_from_snowflake(df2_query)
    reference_df_query = "SELECT * FROM PRODUCT_DIMENSIONS_DICT;"
    reference_df = load_data_from_snowflake(reference_df_query)
...
def calculate_and_plot_cpi(df2, asin_list, start_date, end_date, price_min, price_max, compulsory_features, same_brand_option):
    ...
    try:
        ...
        ads_data_query = "SELECT * FROM NAPQUEEN_ADS_DATA;"
        napqueen_df = load_data_from_snowflake(ads_data_query)
